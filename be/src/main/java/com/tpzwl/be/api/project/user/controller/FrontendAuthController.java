package com.tpzwl.be.api.project.user.controller;
...
import com.tpzwl.be.api.project.user.service.UserInfoService;
import com.tpzwl.be.api.project.user.service.UserMenuService;

/**
 * FrontendAuthController: 为前端定制的处理用户认证和注册等等的控制器。
 * 相对应的com.tpzwl.be.api.project.controller.AuthController是缺省的用于处理用户认证和注册等等的控制器。
 */
...
public class FrontendAuthController {
	...
	@PostMapping("/login")
	public ResponseEntity<Response<LoginResponse>> authenticateUser(@Valid @RequestBody LoginRequest loginRequest) {
		...
		// TODO: 往缓存保存token，生产环境可以是Redis，开发环境可以是Caffeine
		...
		return ResponseEntity.ok().body(new Response<LoginResponse>(20000L, "success", loginResponse));
	}
	...
	public ResponseEntity<?> logoutUser(HttpServletRequest request) {
		...
		// TODO: 从缓存删除token，生产环境可以是Redis，开发环境可以是Caffeine
...
//				.body(new MessageResponse("You've been signed out!"));
	...
	public ResponseEntity<?> info(HttpServletRequest request) {
		...
		if (principle.toString() != "anonymousUser") {
			String name = ((UserDetailsImpl) principle).getUsername();
			userInfoResponse = userInfoService.getUserInfoByName(name);
		}

		return ResponseEntity.ok(new Response<UserInfoResponse>(20000L, "User registered successfully!", userInfoResponse));
	}
	...
	public ResponseEntity<?> menu(HttpServletRequest request) {
		...
		if (principle.toString() != "anonymousUser") {
			String name = ((UserDetailsImpl) principle).getUsername();
			menuResponseList = userMenuService.getUserMenusByName(name);
		}
...
//				.body(new MessageResponse("You've been signed out!"));
		...
		return ResponseEntity.ok(new Response<List<MenuResponse>>(20000L, "User registered successfully!", menuResponseList));
	}
...
//		return ResponseEntity.badRequest().body(new MessageResponse("Refresh Token is empty!"));
	...
	@PostMapping("/error")
	public ResponseEntity<List<MenuResponse>> error(HttpServletRequest request) {
		throw new RuntimeException("Test exception from /api/user/error");
	}
